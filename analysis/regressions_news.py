...
model = smf.ols(formula="sentiment_polarity ~ esg_ratio + n_articles_processed", data=df)
results = model.fit()
print(results.summary())

model = smf.ols(
    formula="sentiment_polarity ~ esg_ratio_env + esg_ratio_soc + esg_ratio_gov + n_articles_processed", data=df
)
results = model.fit()
print(results.summary())

model = smf.ols(formula="sentiment_polarity ~ esg + n_articles_processed + esg_ratio", data=df)
results = model.fit()
print(results.summary())

model = smf.ols(
    formula="sentiment_polarity ~ esg + n_articles_processed + esg_ratio_env + esg_ratio_soc + esg_ratio_gov", data=df
)
results = model.fit()
print(results.summary())
